...
from autoreduce_frontend.reduction_viewer.views import (run_queue, run_summary, runs_list, fail_queue, run_confirmation, variables, pause,
                                                        configure_new_batch_run, configure_new_runs, rerun_jobs)
...
urlpatterns = [
    ...
    path('<str:instrument_name>/batch/<int:pk>/<int:run_version>/', run_summary.run_summary_batch_run, name='batch_summary'),
    ...
    path('<str:instrument>/variables/<int:start>/<int:end>/delete', variables.delete_instrument_variables, name='delete_variables'),
