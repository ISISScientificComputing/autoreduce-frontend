...
class ReductionRunTable(Table):
    ...
    run_number = tables.TemplateColumn(
        '{% load get_run_navigation_queries %} <a href="{% url \'runs:summary\' record.instrument record.run_number record.run_version %}?page={{current_page}}&per_page={{per_page}}&sort={{ sort }}&filter={{ filtering }}">{{record.title}}</a>',
        attrs={"td": {
            "class": "run-num-links"
        }},
